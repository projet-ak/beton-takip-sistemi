...
async function scanKaydet(canvas,isPdf,file){
    try{
        if(isPdf){ var fd=new FormData(); fd.append('dosya',file); var r=await fetch('../api/demir_pdf_kaydet.php',{method:'POST',body:fd}); var j=await r.json(); return j.ok?j.url:null; }
        ...
        var r=await fetch('../api/demir_scan_kaydet.php',{method:'POST',body:fd}); var j=await r.json(); return j.ok?j.url:null;
    }catch(e){ return null; }
}
