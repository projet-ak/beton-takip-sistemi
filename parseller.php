...
if (isset($_GET['sil']) && ctype_digit($_GET['sil'])) {
    $id = (int)$_GET['sil'];
    $ki = $pdo->prepare("SELECT COUNT(*) FROM irsaliyeler WHERE parsel_id = ?");
    $ki->execute([$id]);
    $kb = $pdo->prepare("SELECT COUNT(*) FROM bloklar WHERE parsel_id = ?");
    $kb->execute([$id]);
    if ($ki->fetchColumn() > 0 || $kb->fetchColumn() > 0) {
        flash('warning', 'Bu parsel blok veya irsaliyelerde kullanılıyor, silinemez.');
    } else {
        $pdo->prepare("DELETE FROM parseller WHERE id = ?")->execute([$id]);
        flash('success', 'Parsel silindi.');
    }
...
if (isset($_GET['duzenle']) && ctype_digit($_GET['duzenle'])) {
    $s = $pdo->prepare("SELECT * FROM parseller WHERE id = ?");
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-map text-primary me-2"></i>Parseller</h4>
    <a href="parseller.php?ekle=1" class="btn btn-primary">
        <i class="bi bi-plus-circle me-1"></i> Yeni Parsel
    </a>
</div>
...
<div class="card mb-4">
    <div class="card-header <?= $duzenle ? 'bg-warning text-dark' : 'bg-primary text-white' ?> fw-semibold">
        ...
        <?= $duzenle ? 'Parsel Düzenle' : 'Yeni Parsel Ekle' ?>
    </div>
    <div class="card-body">
        <?php if ($formError): ?>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end text-nowrap">
                            <a href="parseller.php?duzenle=<?= $r['id'] ?>" class="btn btn-xs btn-outline-primary me-1">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <a href="parseller.php?sil=<?= $r['id'] ?>"
                               class="btn btn-xs btn-outline-danger btn-confirm"
                               data-msg="Bu parseli silmek istediğinize emin misiniz?">
