...
- **`metraj_takip.php`** (`can_view_reports()`, "İmalat Sayfaları" menüsü) — **İmalat/Metraj/Zayiat sayfaları görüntüleyici**: `metraj_sayfa`'daki sayfaları (PRP Bina Üstyapı, İksa/Temel Altı Kazık, İstinat Dener/Duvar, PRP Temel, Kazık, Metraj, Mobilizasyon…) **sekmeler** halinde gösterir. **İçe aktarma bu ekranda YOK** — `import.php` (Dinamik Excel Aktarımı) Excel yüklenince `storeImalatSheets()` ile otomatik doldurur. **KOT ve İCMAL burada gizli** (ayrı ekranlar: `kotlar.php` / `icmal_beton.php`). Sayı hücreleri TR formatlı, #N/A soluk. Admin: sayfa kaldırma.
