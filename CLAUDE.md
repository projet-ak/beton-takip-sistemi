...
- **`whatsapp/` modülü = AYRI MODÜL** (topbar sekmesi **"Saha Takip"**, `$__module='whatsapp'`) — WhatsApp/serbest metin → irsaliye & saha olayı hattı. Beton panelinden ayrıdır; kendi sidebar'ı vardır. Modül girişi yetkiye göre: `can_edit()` varsa mesajlar.php, yoksa saha_analiz.php (`$__waHome`).
  - **`whatsapp/mesajlar.php`** (`can_edit()`) — **Gelen Mesajlar / Onay Kuyruğu**: WhatsApp'tan (veya elle yapıştırma) gelen serbest metinler `mesaj_kuyrugu`'na düşer, AI ile ayrıştırılır, **insan onayından sonra** irsaliye olur (otomatik kayıt YOK; mükerrer irsaliye no engelli). Giriş ucu **`whatsapp/api/mesaj_al.php`** (`MESAJ_TOKEN` korumalı, Meta Cloud API webhook doğrulaması destekli; yol `/api/` içerdiği için CSRF muaf). Ortak katman **`whatsapp/_ortak.php`**.
  - **`whatsapp/saha_analiz.php`** (`can_view_reports()`) — **Saha Hareket Analizi**: aynı mesajlardan AI ile çıkarılan saha olayları (`saha_olaylari`): personel giriş/çıkış, yetkilendirme (veren/alan), araç çalışma saatleri, iş bildirimleri. KPI + günlük hareket grafiği (Chart.js) + kişi/araç/yetki tabloları + zaman çizelgesi (satıra gelince ham mesaj). Güven skoru <%60 kırmızı. **Resmî puantaj/İSG kaydı değildir** (uyarı bandı var).
