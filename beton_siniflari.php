...
$pageTitle = 'Beton Sınıfları — Beton Takip Sistemi';

// ── Sil ──────────────────────────────────────────────────────────────────────
if (isset($_GET['sil']) && ctype_digit($_GET['sil'])) {
    $id = (int)$_GET['sil'];
    $k = $pdo->prepare("SELECT COUNT(*) FROM irsaliyeler WHERE beton_sinifi_id = ?");
    $k->execute([$id]);
    if ($k->fetchColumn() > 0) {
        flash('warning', 'Bu beton sınıfı irsaliyelerde kullanılıyor, silinemez.');
    } else {
        $pdo->prepare("DELETE FROM beton_siniflari WHERE id = ?")->execute([$id]);
        flash('success', 'Beton sınıfı silindi.');
    }
    redirect('beton_siniflari.php');
}
...
if (isset($_GET['duzenle']) && ctype_digit($_GET['duzenle'])) {
    $s = $pdo->prepare("SELECT * FROM beton_siniflari WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$ad) {
        ...
        $dupSql = $id
            ? "SELECT COUNT(*) FROM beton_siniflari WHERE UPPER(ad) = UPPER(?) AND id != ?"
            : "SELECT COUNT(*) FROM beton_siniflari WHERE UPPER(ad) = UPPER(?)";
        $dupStmt = $pdo->prepare($dupSql);
        $dupStmt->execute($id ? [$ad, $id] : [$ad]);
        if ($dupStmt->fetchColumn() > 0) {
            $formError = '"' . h($ad) . '" adında bir beton sınıfı zaten mevcut. Lütfen farklı bir ad girin.';
        } else {
            if ($id) {
                $pdo->prepare("UPDATE beton_siniflari SET ad = ? WHERE id = ?")->execute([$ad, $id]);
                flash('success', 'Beton sınıfı güncellendi.');
            } else {
                $pdo->prepare("INSERT INTO beton_siniflari (ad) VALUES (?)")->execute([$ad]);
                flash('success', 'Beton sınıfı eklendi.');
            }
            redirect('beton_siniflari.php');
        }
    }
...
$liste = $pdo->query("
    SELECT b.*,
           (SELECT COUNT(*) FROM irsaliyeler WHERE beton_sinifi_id = b.id) AS irsaliye_sayisi
    FROM beton_siniflari b ORDER BY b.ad
")->fetchAll();
$duplar = $pdo->query(
    "SELECT UPPER(ad) as ad_upper FROM beton_siniflari GROUP BY UPPER(ad) HAVING COUNT(*) > 1"
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-layers text-primary me-2"></i>Beton Sınıfları</h4>
    <a href="beton_siniflari.php?ekle=1" class="btn btn-primary">
        <i class="bi bi-plus-circle me-1"></i> Yeni Beton Sınıfı
    </a>
</div>
...
<div class="alert alert-warning d-flex align-items-start gap-2">
    ...
    <div>
        ...
        Listede aynı isimli birden fazla beton sınıfı var: <strong><?= implode(', ', array_map('h', $duplar)) ?></strong>.
...
<div class="card mb-4">
    <div class="card-header <?= $duzenle ? 'bg-warning text-dark' : 'bg-primary text-white' ?> fw-semibold">
        ...
        <?= $duzenle ? 'Beton Sınıfı Düzenle' : 'Yeni Beton Sınıfı Ekle' ?>
    </div>
    <div class="card-body">
        <?php if ($formError): ?>
        ...
        <form method="post">
            ...
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-6 d-flex gap-2">
                    ...
                    <a href="beton_siniflari.php" class="btn btn-secondary"><i class="bi bi-x-circle me-1"></i>İptal</a>
                </div>
            </div>
        </form>
    </div>
                ...
                <tbody>
                    ...
                    <tr <?= $isDup ? 'class="table-warning"' : '' ?>>
                        ...
                        <td class="text-center">
                            <?php if ($r['irsaliye_sayisi'] > 0): ?>
                                <a href="#" class="badge bg-primary text-white text-decoration-none btn-tanim-modal"
                                   data-tip="beton" data-id="<?= $r['id'] ?>" data-ad="<?= h($r['ad']) ?>">
                        ...
                        <td class="text-end text-nowrap">
                            <a href="beton_siniflari.php?duzenle=<?= $r['id'] ?>" class="btn btn-xs btn-outline-primary me-1">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <a href="beton_siniflari.php?sil=<?= $r['id'] ?>"
                               class="btn btn-xs btn-outline-danger btn-confirm"
                               data-msg="Bu beton sınıfını silmek istediğinize emin misiniz?">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$liste): ?>
                        <tr><td colspan="3" class="text-center text-muted py-5">Henüz beton sınıfı yok.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
                ...
                <div id="modalIcerik" class="d-none">
                    <table class="table table-hover table-sm align-middle mb-0">
                        <thead class="table-light sticky-top">
                            <tr>
                                <th>İrsaliye No</th><th>Tarih</th><th>Proje</th><th>Araç Plaka</th>
    ...
    btn.addEventListener('click', function(e) {
        ...
        fetch(`api/tanim_irsaliyeler.php?tip=${tip}&id=${id}`)
            .then(r => r.json()).then(data => {
                ...
                tbody.innerHTML = '';
                if (!data.ok) { tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-3">${data.msg}</td></tr>`; }
                else if (data.liste.length === 0) { tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-4">Kayıt bulunamadı.</td></tr>'; }
