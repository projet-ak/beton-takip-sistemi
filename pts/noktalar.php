...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($islem === 'kaydet') {
            ...
            $lok = trim((string)($_POST['lokasyon'] ?? '')) ?: null;
            ...
            if ($id > 0) {
                $pdoPts->prepare("UPDATE pts_noktalar SET kod=?, ad=?, yon=?, lokasyon=?, aktif=? WHERE id=?")
                ...
                $pdoPts->prepare("INSERT INTO pts_noktalar (kod, ad, cihaz_anahtari, yon, lokasyon, aktif, created_at)
                                  VALUES (?,?,?,?,?,1,?)")
...
// Lokasyon serbest METİNDİR (başka modülün lokasyon ağacına bağlanmaz) — öneri listesi
// hem personel kartlarından hem daha önce girilmiş nokta lokasyonlarından toplanır.
$lokSecenek = pts_personel_secenekler($pdoPts, 'lokasyon');
foreach ($liste as $__n) if (!empty($__n['lokasyon']) && !in_array($__n['lokasyon'], $lokSecenek, true)) $lokSecenek[] = $__n['lokasyon'];
sort($lokSecenek);
    ...
    <table class="table table-hover table-sm align-middle mb-0" style="font-size:.86rem">
      ...
      <?php foreach ($liste as $n): ?>
        <tr class="<?= $n['aktif'] ? '' : 'text-muted' ?>">
          ...
          <td class="small"><?= h($n['lokasyon'] ?: '—') ?></td>
          ...
          <td class="text-end text-nowrap">
            ...
            <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#noktaModal"
                    ...
                    data-yon="<?= h($n['yon']) ?>" data-lok="<?= h((string)$n['lokasyon']) ?>" data-aktif="<?= (int)$n['aktif'] ?>">
  ...
  <div class="modal-dialog"><div class="modal-content"><form method="post">
    ...
    <div class="modal-body">
      ...
      <div class="mb-2"><label class="form-label small">Lokasyon</label>
        <input name="lokasyon" id="nmLok" class="form-control form-control-sm" list="ptsLokListe"
               placeholder="Kartal Batı Yakası — 1. Etap">
        <datalist id="ptsLokListe"><?php foreach ($lokSecenek as $l): ?><option value="<?= h($l) ?>"><?php endforeach; ?></datalist></div>
...
if (nm) nm.addEventListener('show.bs.modal', function(ev){
  ...
  document.getElementById('nmLok').value   = yeni ? '' : (b.dataset.lok || '');
