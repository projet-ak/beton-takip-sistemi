...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$ad) {
        ...
        $dupSql = $id
            ? "SELECT COUNT(*) FROM pompa_turleri WHERE UPPER(ad) = UPPER(?) AND id != ?"
            : "SELECT COUNT(*) FROM pompa_turleri WHERE UPPER(ad) = UPPER(?)";
        $dupStmt = $pdo->prepare($dupSql);
        $dupStmt->execute($id ? [$ad, $id] : [$ad]);
        if ($dupStmt->fetchColumn() > 0) {
            $formError = '"' . h($ad) . '" adında bir pompa türü zaten mevcut. Lütfen farklı bir ad girin.';
        } else {
            if ($id) {
...
<div class="alert alert-warning d-flex align-items-start gap-2">
    ...
    <div>
        ...
        Listede aynı isimli birden fazla pompa türü var: <strong><?= implode(', ', array_map('h', $duplar)) ?></strong>.
