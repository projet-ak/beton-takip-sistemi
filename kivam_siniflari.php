...
$liste = $pdo->query("
    SELECT k.*,
           (SELECT COUNT(*) FROM irsaliyeler WHERE kivam_sinifi_id = k.id) AS irsaliye_sayisi
    FROM kivam_siniflari k ORDER BY k.ad
")->fetchAll();
$duplar = $pdo->query(
                        ...
                        <td class="text-center">
                            <?php if ($r['irsaliye_sayisi'] > 0): ?>
                                <a href="#" class="badge bg-primary text-white text-decoration-none btn-tanim-modal"
                                   data-tip="kivam" data-id="<?= $r['id'] ?>" data-ad="<?= h($r['ad']) ?>">
